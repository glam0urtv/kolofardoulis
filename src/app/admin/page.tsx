...
export default async function AdminDashboard() {
  ...
  const lowStock = products.filter((p) => (p.inventory?.stock ?? 0) <= 2)
      ...
      <div className="grid gap-6 lg:grid-cols-2">
              ...
              lowStock.map((item) => (
                ...
                >
                  ...
                  <span className="rounded-full bg-red-50 px-2.5 py-0.5 text-xs font-bold text-red-600">
                    {item.inventory?.stock ?? 0} τεμάχια
                  </span>
                </div>
              ))
            )}
        ...
        <div className="rounded-2xl border border-stone-200 bg-white p-6">
          ...
          <div className="mt-4 grid gap-3">
            <Link
              href="/admin/products"
              className="flex items-center gap-3 rounded-xl border border-stone-200 p-4 transition-colors hover:bg-stone-50"
            >
